...
use Duplicator\Package\Create\Scan\ArchiveNodesList;
...
class DUP_PRO_Package
{
    ...
    protected static function get_from_json($json, $rowData = null)
    {
        $map = new JsonUnserializeMap(
            [
                ...
                'Archive/listDirObj'                         => '?cl:' . ArchiveNodesList::class,
                'Archive/listFileObj'                        => '?cl:' . ArchiveNodesList::class,
     ...
     */
    public function createScanReport()
    {
        global $wpdb;
        ...
        try {
            ...
            $filteredTables        = ($this->Database->FilterOn ? explode(',', $this->Database->FilterTables) : array());
            $subsites              = DUP_PRO_MU::getSubsites($this->Multisite->FilterSites, $filteredTables);
            $hasImportableSites    = SnapUtil::inArrayExtended($subsites, function ($subsite) {
                    return count($subsite->filteredTables) === 0;
            });
            $hasNotImportableSites = SnapUtil::inArrayExtended($subsites, function ($subsite) {
                    return count($subsite->filteredTables) > 0;
            });
            ...
            if (is_multisite() && License::can(License::CAPABILITY_MULTISITE_PLUS)) {
                $filteredSites = array_map(
                    function ($siteId) {
                        return get_blog_details(array('blog_id' => $siteId));
                    },
    ...
    public function getScanReportFromJson($json_path)
    {
        ...
        $fileListObj        = new ArchiveNodesList($fileListPath);
        $report->ARC->Files = $fileListObj->getArrayPaths($targetRootPath);
        $dirListObj         = new ArchiveNodesList($dirListPath);
    ...
    protected function make_hash()
    {
        ...
        try {
            ...
            if (function_exists('random_bytes')) {
                ...
                return strtolower(md5(uniqid((string) rand(), true))) . '_' . $date;
            }
        } catch (Exception $exc) {
            return strtolower(md5(uniqid((string) rand(), true))) . '_' . $date;
        }
