...
$progress_html    = "<span style='display:none' class='status-{$package->ID}'>{$status}</span>";
$stop_button_text = __('Stop', 'duplicator-pro');

if ($status >= 0) {
    if ($status >= 75) {
        $stop_button_text = __('Stop Transfer', 'duplicator-pro');
        $progress_html    = "<i class='fa fa-sync fa-sm fa-spin'></i>&nbsp;<span class='status-progress-{$package->ID}'>0</span>%"
        . "<span style='display:none' class='status-{$package->ID}'>{$status}</span>";
    } elseif ($status > 0) {
        $stop_button_text = __('Stop Build', 'duplicator-pro');
        $progress_html    = "<i class='fa fa-cog fa-sm fa-spin'></i>&nbsp;<span class='status-{$package->ID}'>{$status}</span>%";
    } else {
        // In a pending state
